...
export default function HomePage() {
  ...
  // Toast — used to confirm "Created", "Updated", etc. across SPA navigations.
  // The standalone /app/new-product wizard hands off via sessionStorage so the
  // message lives across the navigate() call without polluting the URL.
  const [toast, setToast] = useState(null);
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("pendora_toast");
      if (raw) {
        sessionStorage.removeItem("pendora_toast");
        const parsed = JSON.parse(raw);
        if (parsed && parsed.message) setToast(parsed);
      }
    } catch {}
  }, []);
  useEffect(() => {
    if (!toast) return;
    const dur = typeof toast.duration === "number" ? toast.duration : 3000;
    const id = setTimeout(() => setToast(null), dur);
  ...
  const handleWizardSubmit = async () => {
    ...
    try {
      ...
      // Confirm to the user before the loader-driven skeleton fills in.
      setToast({ message: `Your digital product "${wProduct?.title || "product"}" has been created`, type: "success" });
      setMode("view");
    } catch (err) {
  ...
  return wrap(
    ...
    <style>{`
      ...
      @keyframes pendora-toast-in { from { opacity: 0; transform: translateY(-12px); } to { opacity: 1; transform: translateY(0); } }
      .pendora-noscroll::-webkit-scrollbar { display: none; }
    `}</style>

    {/* Toast overlay — confirms Created/Updated while the loader fills in fresh data behind it */}
    {toast && (
      <div
        role="status"
        aria-live="polite"
        style={{
          ...
          background: t.text, color: "#fff", padding: "12px 20px", borderRadius: "10px",
          fontSize: "14px", fontWeight: 600, boxShadow: "0 8px 24px rgba(0,0,0,0.18)",
          display: "flex", alignItems: "center", gap: "10px", zIndex: 9999,
          maxWidth: "min(520px, 90vw)", animation: "pendora-toast-in 0.22s ease-out",
        }}
      >
        <span
          style={{
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
      ...
      <div className="pendora-noscroll" style={{ flex: 1, display: "flex", flexDirection: "column", overflow: mode === "create" ? "hidden" : "auto", scrollbarWidth: "none", msOverflowStyle: "none", background: t.bg, transition: TRD, minHeight: 0 }}>
        ...
        {mode === "view" && showSkeleton(isRevalidating, selected) && (
          <div style={{ padding: "clamp(16px, 4vw, 24px) clamp(14px, 4vw, 28px)" }}>
        ...
        {mode === "view" && visibleProducts.length > 0 && !showSkeleton(isRevalidating, selected) && (
          <div style={{ padding: "clamp(16px, 4vw, 24px) clamp(14px, 4vw, 28px)" }}>
            ...
            <div style={{ position: "relative", marginBottom: "14px", maxWidth: "420px", width: "100%" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {renderProducts.map((product) => (
                <div key={product.productId} style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: "12px", boxShadow: t.shadow, overflow: "hidden" }}>
                  {/* Card header — wraps action buttons below product title on narrow viewports */}
                  <div style={{ padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px", flexWrap: "wrap", borderBottom: product.files.length ? `1px solid ${t.border}` : "none" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", flex: "1 1 200px", minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: "14px", color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{product.productTitle}</div>
                  ...
                  {/* File rows — keep nested indent on desktop, drop it on mobile */}
                  {product.files.length > 0 && product.files.map((file, idx) => (
                    <div key={file.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 16px 10px clamp(16px, 4vw, 68px)", borderBottom: idx < product.files.length - 1 ? `1px solid ${t.border}` : "none", background: "#FAFAFA", flexWrap: "wrap" }}>
                      <div style={{ color: t.faint, flexShrink: 0 }}>{Ic.file(14)}</div>
                      <span style={{ fontSize: "13px", fontWeight: 500, color: t.text, flex: "1 1 140px", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", wordBreak: "break-all" }}>{file.fileName}</span>
                      ...
                      <span style={{ fontSize: "12px", color: t.muted, flexShrink: 0 }}>{file.fileSize}</span>
                    </div>
                  ))}
                </div>
              ))}
    ...
    {/* Confirmation modal */}
    {confirmDelete && (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: "16px", boxSizing: "border-box", overflowY: "auto" }}>
        <div style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: "16px", padding: "clamp(20px, 5vw, 28px) clamp(20px, 5vw, 32px)", maxWidth: "420px", width: "100%", boxShadow: "0 8px 40px rgba(0,0,0,0.35)", boxSizing: "border-box", maxHeight: "calc(100dvh - 32px)", overflowY: "auto" }}>
...
function Wizard({ inp, Ic, t, step, setStep, search, setSearch, wProduct, setWProduct, wFiles, setWFiles, wExisting, setWExisting, allExistingFiles, wSubmitting, wError, wCanRetry, wDonePartsRef, wProgressText, filteredShopify, shopifyLoading, shopifyLoadingMore, loadMoreProducts, wizardFileInputRef, wizardPickFiles, handleWizardSubmit, onCancel }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ flexShrink: 0, padding: "clamp(12px, 3vw, 28px) clamp(12px, 3vw, 40px) clamp(10px, 2.5vw, 20px)", borderBottom: `1px solid ${t.border}` }}>
        <div style={{ display: "flex", alignItems: "flex-start", padding: "0 2px" }}>
          {WIZARD_STEPS.flatMap((label, i) => {
            ...
            const col = (
              <div key={`s${num}`} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: "0 0 auto", width: "clamp(72px, 22vw, 110px)" }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: isDone ? t.stepDone : isActive ? t.stepActive : t.border, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "13px", color: (isDone || isActive) ? "#fff" : t.muted, boxShadow: isActive ? `0 0 0 5px ${t.pill}` : "none", flexShrink: 0 }}>
                  {isDone ? Ic.check(13) : num}
                </div>
                <span style={{ fontSize: "11px", marginTop: "7px", color: isActive ? t.text : t.muted, fontWeight: isActive ? 700 : 500, textAlign: "center", lineHeight: 1.25, letterSpacing: "0.1px" }}>{label}</span>
              </div>
            );
            if (i === WIZARD_STEPS.length - 1) return [col];
            return [
              col,
              <div key={`l${num}`} style={{ flex: 1, height: "2px", background: step > num ? t.stepDone : t.stepLine, margin: "17px 6px 0", borderRadius: "2px", minWidth: 12 }} />,
            ];
          })}
        </div>
      </div>
      ...
      <div className="pendora-noscroll" style={{ flex: 1, overflowY: "auto", padding: "clamp(18px, 4vw, 28px) clamp(14px, 4vw, 40px)", minHeight: 0, scrollbarWidth: "none" }}>
      ...
      <div style={{ flexShrink: 0, padding: "clamp(10px, 2.5vw, 14px) clamp(14px, 4vw, 40px) clamp(12px, 3vw, 20px)", borderTop: `1px solid ${t.border}`, background: t.bg, display: "flex", justifyContent: "space-between", alignItems: "center", gap: "10px" }}>
        {renderFooter()}
      </div>
